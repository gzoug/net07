 ...
 */
 public class Base64 {
	...
	ByteArrayOutputStream data = new ByteArrayOutputStream(inData.length());
	...
	if (i < inData.length()) /* then '=' found, but the end of string */
            switch(state)
            {   case 3: data.write(output,0,1);
                    return (inData.charAt(i)=='=') && (inData.charAt(i+1)=='=')
                    	 ? data.toByteArray() : null;
            	case 4: data.write(output,0,2);
                    return (inData.charAt(i)=='=') ? data.toByteArray() : null;
            	default: return null;
            }
     ...
     *  @param inData binary data as byte array
	...
	StringBuffer encoded = new StringBuffer();
    	...
    	{
            int ic = (inData[i] >= 0 ? inData[i] : (inData[i] & 0x7F) + 128);
            switch (state)
